...
transformed parameters {
  ...
  synthesis_over_degradation = asymptotic_normalized_state * max_expression;
...
model {
    ...
    for(m in 1:num_measurements) {
      real sigma;
      sigma = measurement_sigma_absolute + measurement_sigma_relative * predicted_expression[m];
      expression[m] ~ normal(predicted_expression[m], sigma);
    }
...
generated quantities {
  ...
  for(m in 1:num_measurements) {
    ...
    real lower_bound;
     lower_bound = 0;
    real p;
     p= normal_cdf(lower_bound, predicted_expression[m], sigma);
    real u;
     u= uniform_rng(p, 1);
